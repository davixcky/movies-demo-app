...
export const DetailsPage = ({movieID}: DetailsPageProps) => {
    ...
    const getGenres = () => {
        ...
        return <div className={styles.genres_container}>
    ...
    const trailer = useMemo(() => {
        ...
        return <ReactPlayer url={`https://www.youtube.com/watch?v=${trailer.key}`} muted loop playing
                            controls={false} width='100%'
                            height='100%'/>
    }, [movieDetail]);
    ...
    return (
        <div className={styles.container}>
            <div className={styles.details_container}>
                <Button leftIcon={<ArrowLeft/>} onClick={goBack}>Go back</Button>
                <div className={styles.details}>
                    <p className={styles.movie_title}>{movieDetail.data.data.title}</p>
                    <p className={styles.movie_duration}>{movieDetail.data.data.release_date.slice(0, 4)} - 1h
                        45m</p>
                    {
                        getGenres()
                    }
            ...
            <div className={styles.extra_details_container}>
                ...
                <div className={styles.extra_details}>
                    <p>Director: <span>{directors.map(d => d.name).join(', ')}</span></p>
                    <p>Writer: <span>{writers.map(d => d.name).join(', ')}</span></p>
                    <p>Cast: <span>{mainCast.map(c => c.name).join(', ')}</span></p>
                </div>
