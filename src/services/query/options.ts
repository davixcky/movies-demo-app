...
import {
    ...
    KEY_LIST_GENRES, KEY_TRENDING_MOVIES
} from "../const.ts";
import {getCastByMovieId, getGenres, getMovieById, getRecommendationByMovieId, getTrendingMovies} from "../movies.ts";
...
export const movieDetailsQueryOptions = (movieID: string) => queryOptions({
    queryKey: KEY_GET_MOVIE_BY_ID(movieID),
    queryFn: () => getMovieById(movieID),
});

export const movieCastQueryOptions = (movieID: string) => queryOptions({
    queryKey: KEY_GET_CAST_BY_MOVIE_ID(movieID),
    queryFn: () => getCastByMovieId(movieID),
});

export const movieRecommendationsQueryOptions = (movieID: string) => queryOptions({
