...
const MovieGenres = ({movie, genres, maxGenreDisplay}: MovieProps) => {
    ...
    if (movie.genres && movie.genres.length > 0) {
        return (
            <div className={styles.movie_genre_container}>
                {movie.genres.slice(0, maxGenreDisplay).map((genre) => <MovieGenre key={genre.id} genre={genre}/>)}
            </div>
        );
    }
    ...
    return (
        <div className={styles.movie_genre_container}>
            {genresList.map((genre) => <MovieGenre key={genre.id} genre={genre}/>)}
        </div>
    );
}
...
export const Movie = ({movie, genres, maxGenreDisplay}: MovieProps) => {
    ...
    return (
        <div key={movie.id} className={styles.container} onClick={() => navigate({
            ...
            <MovieGenres movie={movie} genres={genres} maxGenreDisplay={maxGenreDisplay}/>
